...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if language == 'ru':
        if has_excel and excel_data:
            system_prompt = f'''Ты эксперт по Excel. У пользователя есть Excel файл с данными. Создай формулу на основе его запроса.
...
ВАЖНО:
- Анализируй данные в файле: какие столбцы заполнены, какие значения там есть
- Создавай формулу, которая работает с РЕАЛЬНЫМИ столбцами из файла (не A, если данные в B!)
- Если запрос "просуммируй столбец с данными" - найди столбец с числами и используй его (например B, если там данные)
...
{{
  "formula": "=СУММ(B:B)",
  "explanation": "подробное объяснение формулы",
  "functions": [{{"name": "СУММ", "description": "суммирует значения"}}]
}}

ОБЯЗАТЕЛЬНО используй РУССКИЕ названия функций (СУММ вместо SUM, ЕСЛИ вместо IF, ОСТАТ вместо MOD).'''
        else:
